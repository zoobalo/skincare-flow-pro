...
export const pmRoutes = new Hono()
  ...
  .patch("/:id", async (c) => {
    try {
      const body = await c.req.json();
      const { code, name, category, description, image, currentStock, minThreshold, moq, leadTimeDays, costPerUnit, docsLink } = body;
      const [updated] = await updatePm(c.req.param("id"), { code, name, category, description, image, currentStock, minThreshold, moq, leadTimeDays, costPerUnit, docsLink });
