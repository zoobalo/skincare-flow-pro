...
export const productionRoutes = new Hono()
  ...
  .post("/", async (c) => {
    try {
      ...
      const { batchNumber, skuId, manufacturerId, quantity, currentStage, startedAt, expectedCompletion, delayed, materialCategory, materialItemId, materialItemName, applicableStages } = body;
      const [created] = await createBatch({ id: crypto.randomUUID(), batchNumber, skuId, manufacturerId, quantity, currentStage, startedAt, expectedCompletion, delayed: delayed ?? false, materialCategory: materialCategory ?? null, materialItemId: materialItemId ?? null, materialItemName: materialItemName ?? null, applicableStages: applicableStages ?? null });
  ...
  .patch("/:id", async (c) => {
    try {
      const body = await c.req.json();
      const { batchNumber, skuId, manufacturerId, quantity, currentStage, startedAt, expectedCompletion, delayed, materialCategory, materialItemId, materialItemName, applicableStages } = body;
      const [updated] = await updateBatch(c.req.param("id"), { batchNumber, skuId, manufacturerId, quantity, currentStage, startedAt, expectedCompletion, delayed, materialCategory: materialCategory ?? null, materialItemId: materialItemId ?? null, materialItemName: materialItemName ?? null, applicableStages: applicableStages ?? null });
