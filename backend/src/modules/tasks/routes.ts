...
export const taskRoutes = new Hono()
  .get("/", async (c) => {
    return c.json(await getAllTasks());
  })
  .post("/", async (c) => {
    try {
      const { title, comments, status, urgency, skuId, productType, deadlineDate } = await c.req.json();
      const [created] = await createTask({ id: crypto.randomUUID(), title, comments: comments ?? "", status: status ?? "None", urgency: urgency ?? "Medium", skuId: skuId ?? null, productType: productType ?? "None", deadlineDate: deadlineDate ?? null });
