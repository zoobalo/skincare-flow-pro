...
import { vendors, skus, helpers } from "@/lib/mock/data";
...
function NewPOWizard() {
  ...
  const submit = () => navigate({ to: "/purchase-orders" });
  const sku = helpers.sku(skuId);
  const vendor = helpers.vendor(vendorId);
      ...
      <div className="rounded-xl border bg-card p-6">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={material} onValueChange={setMaterial}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>{sku?.packaging.map(p => <SelectItem key={p.id} value={p.name}>{p.name}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {vendors.slice(0, 6).map(v => (
                <button key={v.id} onClick={() => setVendorId(v.id)} className={cn("rounded-lg border bg-background p-4 text-left transition-all hover:border-primary", vendorId === v.id && "border-primary ring-2 ring-primary/20")}>
        ...
        <div className="mt-6 flex items-center justify-between border-t pt-4">
          ...
          {step < steps.length - 1 ? (
            ...
            <Button onClick={submit}><Mail className="mr-1.5 h-4 w-4" />Send PO to vendor</Button>
          )}
        </div>
      </div>
    </div>
