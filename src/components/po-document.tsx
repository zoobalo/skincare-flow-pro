...
export function PODocument(props: PODocumentProps) {
  ...
  return (
    ...
    >
      {/*
        po-layout-table: outer layout table whose <thead> repeats on every
        printed page via `display: table-header-group`. No position:fixed needed.
        @page { margin: 0 } suppresses browser URL/date/page-number text.
        Print CSS adds padding-top to the thead <td> so each page has a top margin.
      */}
      <table className="po-layout-table" style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            <td style={{ padding: 0 }}>
        ...
        <tbody>
          <tr>
            <td style={{ padding: 0 }}>
              ...
              {/* Particulars table — po-items-table scopes the row break rule */}
              <table className="po-items-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                <thead>
...
export function buildPoHtml(props: PODocumentProps): string {
  ...
  const dataRows = rows.map((row, idx) => `<tr>
    <td style="${cellB};text-align:center">${idx + 1}</td>
    <td style="${cellB}"><div style="font-weight:500">${row.description}</div></td>
    <td style="${cellB};text-align:center">—</td>
    <td style="${cellB};text-align:right">${(row.quantity ?? 0).toLocaleString("en-IN")}</td>
    <td style="${cellB};text-align:right">${fmt(row.rate ?? 0)}</td>
    <td style="${cellB};text-align:right">${fmt(row.subtotal ?? 0)}</td>
    <td style="${cellB};text-align:center">${row.gstRate ?? 0}%</td>
    <td style="${cellB};text-align:right">${fmt(row.gstAmount ?? 0)}</td>
    <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:600">${fmt(row.total ?? 0)}</td>
  </tr>`).join("");
  ...
  const termsHtml = terms
    ? `<div style="border-top:1px solid #e5e7eb;padding:12px 16px;font-size:11px;page-break-inside:avoid;break-inside:avoid"><div style="font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#555;font-size:10px;margin-bottom:6px">Terms &amp; Conditions</div><div style="white-space:pre-line;line-height:1.6;color:#444">${terms.replace(/&/g, "&amp;").replace(/</g, "&lt;")}</div></div>`
    : "";
...
body{font-family:Arial,sans-serif;font-size:13px;color:#111;background:#fff;padding:0 12mm 10mm}
@page{size:A4;margin:0}
table{border-collapse:collapse}
.po-layout-table{width:100%}
.po-layout-table>thead{display:table-header-group}
.po-layout-table>thead>tr>td{padding-top:8mm}
.po-items-table{width:100%}
.po-items-table>thead{display:table-header-group}
.po-items-table>tbody>tr{page-break-inside:avoid;break-inside:avoid}
</style>
</head><body>
<table class="po-layout-table">
  <thead>
    <tr>
      <td style="padding-top:8mm">
        <div style="border-bottom:1px solid #e5e7eb;padding:16px 32px;text-align:center">
  ...
  <tbody>
    <tr>
      <td style="padding:0">
        ...
        <table class="po-items-table" style="font-size:11px">
        ...
        <div style="border-top:1px solid #e5e7eb;padding:8px 16px;font-size:11px;page-break-inside:avoid;break-inside:avoid">
          <span style="color:#777">Amount in Words: </span>
